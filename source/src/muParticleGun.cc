...
muParticleGun::muParticleGun()
    ...
    double totalFlux = 0;
    for (decltype(muonFlux.size()) i = 0;
    i < muonFlux.size();
    i++){
    ...
    totalFlux = 0;
    for (decltype(fissionFlux.size()) i = 0;
    i < fissionFlux.size();
    i++){
...
void muParticleGun::GeneratePositron(G4PrimaryParticle *positron[1]) {
    ...
    positron[0]->SetKineticEnergy(
            std::sqrt(electron_mass_c2 * electron_mass_c2 + pep * pep) - electron_mass_c2); // kinetic energy
}
